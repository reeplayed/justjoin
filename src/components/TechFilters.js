...
const TechFilters = ({params}) => {
    ...
    const [mobileView, setMobileView] = useState(getWidth()<=1025 ? true : false);
    ...
    const DesktopView = () =>(
        <Container>
           ...
           {techArray.map((tech)=><TechIcon tech={tech} onclick={onClose}/>)}      
        </Container>
    )
...
const AllIconContainer = styled.div`
    ...
    height:35px;
    border-radius:50px;
    background: ${({theme,focus})=>focus ? theme.techColors['all'] : theme.techColors.disabled};
    overflow: hidden;
    color: #fff;
    text-align: center;
    display:flex;
